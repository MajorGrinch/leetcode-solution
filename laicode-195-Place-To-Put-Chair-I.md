...
代码优化一下，
+ `O`是不可通行的，`C`和`E`都是可以通行的，但是只有`C`可以放置椅子。所以我们算每个`E`到其他`C`距离的时候，需要把其他`E`也放入 BFS 的队列。而`O`点可以不做处理，距离直接设置为 0，不用管。
+ merge 时候什么都不用管，直接合并。
+ 计算全局最短路径的时候，我们只选取`C`点进行对比，就不会受`O`点的 0 值影响。
  ...
  private int[][] getDistance(char[][] gym, int startX, int startY) {
    ...
    while(!qX.isEmpty()) {
      ...
      for(int i = 0; i < dx.length; i++) {
        int nextX = x + dx[i];
        int nextY = y + dy[i];
        if(0 <= nextX && nextX < m && 0 <= nextY && nextY < n && !vis[nextX][nextY] && gym[nextX][nextY] != 'O') {
          qX.offer(nextX);
          qY.offer(nextY);
          vis[nextX][nextY] = true;
