...
参照[laicode 82](laicode-82-Remove-Adjacent-Repeated-Characters-IV.md)。
...
public class Solution {
  public int[] dedup(int[] array) {
    ...
    while(fast < array.length) {
      ...
      if(fast - begin == 1 && (slow == 0 || array[slow - 1] != array[begin])) {
        array[slow++] = array[begin];
      } else if(slow > 0 && array[slow - 1] == array[begin]) {
        slow--;
      }
    }
    int[] res = new int[slow];
